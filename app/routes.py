...
def upload_file():
    ...
    if file and file.filename.endswith('.txt'):
        ...
        return jsonify({"error": "Invalid file format. Supported formats: .txt, .docx, .xlsx"}), 400
...
def download_file():
    ...
    if file_type == 'word':
        ...
        buffer.seek(0)
        return send_file(buffer, as_attachment=True, download_name='redacted_document.docx', mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
        ...
        buffer.seek(0)
        return send_file(buffer, as_attachment=True, download_name='redacted_document.xlsx', mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
